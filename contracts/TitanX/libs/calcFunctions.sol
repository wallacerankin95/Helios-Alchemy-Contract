...
function calculateMintReward(
    ...
    if (numOfDays != 1)
        baseReward -=
            (baseReward * MINT_DAILY_REDUCTION * (numOfDays - 1)) /
            PERCENT_BPS;
...
function calculateMintPowerBonus(
    ...
    bonus = (((mintPowerBonus * mintPower * (globalMintPower - gMintPower)) *
        SCALING_FACTOR_1e18) / MAX_MINT_POWER_CAP);
}
 ...
 */
function calculateClaimMintPenalty(
    uint256 secsLate
) pure returns (uint256 penalty) {
...
function calculateShares(
    ...
    shares +=
        (shares * calculateShareBonus(amount, noOfDays)) /
        SCALING_FACTOR_1e11;
 ...
 */
function calculateShareBonus(
    uint256 amount,
    uint256 noOfDays
) pure returns (uint256 shareBonus) {
    uint256 cappedExtraDays = noOfDays <= LPB_MAX_DAYS
        ? noOfDays
        : LPB_MAX_DAYS;
    uint256 cappedStakedTitan = amount <= BPB_MAX_TITAN
        ? amount
        : BPB_MAX_TITAN;
    shareBonus =
...
function calculateEndStakePenalty(
    ...
    if (currentBlockTs > maturityTs) {
        ...
        return
            max((min((lateSec - gracePeriodSec), 1) / SECONDS_IN_DAY) + 1, 99);
    }
