...
abstract contract MintInfo {
    ...
    mapping(address => mapping(uint256 => TRankInfo))
        private s_addressMIdToTRankInfo;
    ...
    function _startMint(
        ...
        if (numOfDays == 0 || numOfDays > MAX_MINT_LENGTH)
            revert TitanX_InvalidMintLength();
        if (mintPower == 0 || mintPower > MAX_MINT_POWER_CAP)
            revert TitanX_InvalidMintPower();
    ...
    function _batchClaimMint(address user) internal returns (uint256 reward) {
        ...
        for (uint256 i = 1; i <= maxId; i++) {
            ...
            if (
                mint.status == MintStatus.ACTIVE &&
                block.timestamp >= mint.maturityTs
            ) {
    ...
    function _calculateClaimReward(
        ...
        if (action == MintAction.CLAIM)
            s_tRankToMintInfo[tRank].status = MintStatus.CLAIMED;
        if (action == MintAction.BURN)
            s_tRankToMintInfo[tRank].status = MintStatus.BURNED;
        ...
        reward =
            uint256(userMintInfo.mintableTitan) +
        ...
        if (action == MintAction.CLAIM)
            s_tRankToMintInfo[tRank].mintedTitan = uint128(reward);
