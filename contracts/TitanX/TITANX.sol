    ...
    OwnerInfo
{
    ...
    function batchMint(
        ...
        if (count == 0 || count > MAX_BATCH_MINT_COUNT)
            revert TitanX_InvalidBatchCount();
    ...
    function startStake(
        ...
        if (balanceOf(_msgSender()) < amount)
            revert TitanX_InsufficientBalance();
    ...
    function triggerPayouts() external dailyUpdate nonReentrant {
        ...
        _triggerCyclePayout(DAY8, globalActiveShares, currentContractDay) ==
            PayoutTriggered.YES &&
        ...
        _triggerCyclePayout(DAY28, globalActiveShares, currentContractDay) ==
            ...
            ? isTriggered = PayoutTriggered.YES
            : isTriggered;
        _triggerCyclePayout(DAY90, globalActiveShares, currentContractDay) ==
            PayoutTriggered.YES &&
            isTriggered == PayoutTriggered.NO
            ? isTriggered = PayoutTriggered.YES
            : isTriggered;
    ...
    function mintLPTokens() external {
        ...
        if (s_initialLPMinted == InitialLPMinted.YES)
            revert TitanX_LPTokensHasMinted();
        ...
        )
    {
        ...
        incentiveFee =
            ...
            INCENTIVE_FEE_PERCENT_BASE; //0.01%
        accumulatedFees -= incentiveFee;

        buyAndBurnFunds =
            (accumulatedFees * PERCENT_TO_BUY_AND_BURN) /
            PERCENT_BPS;
        uint256 cylceBurnReward = (accumulatedFees * PERCENT_TO_BURN_PAYOUTS) /
            PERCENT_BPS;
        ...
        if (s_burnPoolEnabled == BurnPoolEnabled.TRUE)
            s_cycleBurnReward += uint88(cylceBurnReward);
        else buyAndBurnFunds += cylceBurnReward;

        //cycle payout
        if (cycleRewardPool != 0) {
            uint256 cycle8Reward = (cycleRewardPool * CYCLE_8_PERCENT) /
                PERCENT_BPS;
            uint256 cycle28Reward = (cycleRewardPool * CYCLE_28_PERCENT) /
                PERCENT_BPS;
            uint256 cycle90Reward = (cycleRewardPool * CYCLE_90_PERCENT) /
                PERCENT_BPS;
            uint256 cycle369Reward = (cycleRewardPool * CYCLE_369_PERCENT) /
                PERCENT_BPS;
    ...
    function _triggerCyclePayout(
        ...
        uint256 cycleIndex = _calculateCycleRewardPerShare(
            cycleNo,
            reward,
            globalActiveShares
        );
    ...
    function _burnAfter(
        ...
        if (getUserBurnTotal(user) == 0)
            _updateUserBurnCycleClaimIndex(user, DAY28, index);
    ...
    function _spendBurnStakeAllowance(address user) private {
        ...
        if (currentAllowance != type(uint256).max) {
            if (currentAllowance == 0)
                revert TitanX_InsufficientBurnAllowance();
        ...
        )
    {
        ...
        if (payoutClaim == PayoutClaim.SHARES) {
            ...
            for (uint256 i = userClaimCycleIndex; i <= cycleMaxIndex; i++) {
                ...
                for (
                    uint256 j = userClaimSharesIndex;
                    j <= sharesMaxIndex;
                    j++
                ) {
            ...
            for (uint256 i = userClaimBurnCycleIndex; i <= cycleMaxIndex; i++) {
                ...
                rewards += (burnPayoutPerToken != 0)
                    ? (burnPayoutPerToken * _getUserCycleBurnTotal(user, i)) /
                        SCALING_FACTOR_1e18
                    : 0;
    ...
    function getUserETHClaimableTotal(
        ...
        uint256 _reward;
        (_reward, , , ) = _calculateUserCycleReward(
            user,
            DAY8,
            PayoutClaim.SHARES
        );
     ...
     */
    function getUserBurnPoolETHClaimableTotal(
        address user
    ) public view returns (uint256 reward) {
        (reward, , , ) = _calculateUserCycleReward(
            user,
            DAY28,
            PayoutClaim.BURN
        );
    ...
    function getCurrentUserBurnCyclePercentage() public view returns (uint256) {
        ...
        return
            cycleBurnTotal == 0
                ? 0
                : (_getUserCycleBurnTotal(_msgSender(), index) *
                    100 *
                    SCALING_FACTOR_1e18) / cycleBurnTotal;
    }
