...
abstract contract GlobalInfo {
        ...
        if (currentBlockDay > currentContractDay) {
            ...
            for (uint256 i; i < dayDifference; i++) {
                newMintCost =
                    ...
                    PERCENT_BPS;
                newShareRate =
                    (newShareRate * DAILY_SHARE_RATE_INCREASE_STEP) /
                    PERCENT_BPS;
                newMintableTitan =
                ...
                if (newShareRate > CAPPED_MAX_RATE)
    ...
    function _initFirstSharesCycleIndex(
        ...
        if (isFirstShares == 1) {
            if (s_cyclePayoutIndex[DAY8] != 0) {
                ...
                s_addressCycleToLastClaimIndex[user][DAY369]
                    .cycleIndex = uint96(s_cyclePayoutIndex[DAY369] + 1);

                s_addressCycleToLastClaimIndex[user][DAY888]
                    .cycleIndex = uint96(s_cyclePayoutIndex[DAY888] + 1);
            }
        }
    }
    ...
    function _updateUserClaimIndexes(
        ...
        if (
            ...
            s_addressCycleToLastClaimIndex[user][cycleNo].cycleIndex = uint96(
                userClaimCycleIndex
            );
    ...
    function _updateUserBurnCycleClaimIndex(
        ...
        if (
            ...
            s_addressCycleToLastClaimIndex[user][cycleNo].burnCycleIndex
        )
            s_addressCycleToLastClaimIndex[user][cycleNo]
                .burnCycleIndex = uint96(userClaimBurnCycleIndex);
    }
